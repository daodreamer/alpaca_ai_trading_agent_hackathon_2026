 ...
 * AlphaGate dashboard.
 *
 * Two tabs, answering two questions the other structurally cannot:
 *
 * * **Live** — what is true now. Read from `status.json`, which the agent
 *   rewrites every slot.
 * * **Journal** — what it decided, and why. Read from the append-only JSONL the
 *   submission ships.
 *
 * The page polls rather than holding a socket. A fifteen-minute cycle does not
 * need streaming, a poll survives the agent restarting underneath it, and the
 * staleness of the last snapshot is then a fact the page can *see* rather than
 * a connection state it has to infer.
 */
...
export default function App() {
  ...
  const day =
    chosenDay ?? status?.snapshot?.session_day ?? days[0] ?? todayInBrowser()
    ...
    <div className="bg-background text-foreground min-h-screen">
      <header className="border-border flex flex-wrap items-baseline gap-x-4 gap-y-2 border-b px-6 py-4">
        ...
        <p className="text-muted-foreground text-sm">
          an options agent that can be overruled
        </p>
